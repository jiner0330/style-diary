...
export default function ModelDisplay({ gender, angleIndex: controlledIndex, onAngleChange }: Props) {
  ...
  return (
    <div className={`flex flex-col items-center w-full ${filledCount > 0 ? "py-4 md:py-6 md:pb-6" : "pt-1.5 pb-3 md:py-6 md:pb-6"}`}>
      {filledCount > 0 ? (
        ...
        <div className="flex flex-col items-center gap-1 mb-2 bg-white/75 backdrop-blur-sm rounded-xl px-3 py-1.5 shadow-sm">
          <p className="text-[11px] text-charcoal/80 font-medium tracking-wide leading-tight">
            {isMobile
              ...
              {["挑选衣服", "点击 ✨ 生成", "查看效果"].map((label, i) => (
                <div key={label} className="flex items-center gap-0.5">
                  <span className="flex flex-col items-center">
                    <span className="w-4 h-4 rounded-full bg-charcoal/10 flex items-center justify-center
                                     text-[9px] font-semibold text-charcoal/80">
                      {i + 1}
                    </span>
                    <span className="text-[9px] text-charcoal/70 font-medium mt-0.5 leading-tight">{label}</span>
                  </span>
                  {i < 2 && <span className="text-charcoal/30 text-[10px]">→</span>}
                </div>
              ))}
            </div>
          )}
